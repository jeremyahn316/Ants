...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place != hive or place.entrance != None :
            if len(place.bees) > 0 and transition >= self.min_range and transition <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0: #self.armor - amount
            list_bees = list(self.place.bees)
            for item in list_bees[:]:
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 9
        #implement the BodyguardAnt's action method to perform its contained_ant's 
        #action if it is currently containing an ant
            ...
            while curr.exit:
                ...
                if ant: 
                    ...
                    if isinstance(ant,BodyguardAnt):
                        if ant.contained_ant is not None and ant.contained_ant not in self.doubled:
...
def apply_effect(effect, bee, duration):
    ...
    def alternative_action(colony):
        ...
        if duration:
            duration -= 1
            return effect(original, bee)(colony)
        else:
            return original(colony)
            
    bee.action = alternative_action
